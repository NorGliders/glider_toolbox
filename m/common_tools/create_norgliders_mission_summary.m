function create_norgliders_mission_summary(spreadsheet,summary_json_file)
% create_norgliders_mission_summary  Read in a surface dialog from a slocum log file
% surf_data = create_norgliders_mission_summary(file, glider);
%  Syntax:
%    [ data ] = create_norgliders_mission_summary(x, y)
% function [data, metadata] = getDeploymentInfoXLS(spreadsheet,is_realtime)
...
% Delete some columns
T = removevars(T,{'INTERNALDATAPATH','INTERNALNCFILE','NOTES','STDMISSIONID'});
...
%T.Properties.RowNames = deployments;

Tstruct = table2struct(T);
newTStruct = struct();
for i = 1:numel(deployName)
    if strcmp(deployName{i}(1),'2')
        fieldname = ['x_' deployName{i}];
    else
        fieldname = deployName{i};
    end
        newTStruct.(fieldname) = Tstruct(i);
end

str = jsonencode(newTStruct);
final_str = strrep(str,'x_','');

fid = fopen(summary_json_file,'wt');
fprintf(fid,'%s',final_str);
fclose(fid);
