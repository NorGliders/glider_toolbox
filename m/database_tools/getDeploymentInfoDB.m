...
%  along with this program.  If not, see <http://www.gnu.org/licenses/>.
  ...
  % setdbprefs no longer recommended by mathworks, create an
  % SQLImportOptions object FE 19.01.2021
  ...
  if ~isopen(conn) % (~isconnection(conn))
  ...
  try
    ...
    type = opts.VariableTypes;
    isChar = cellfun(@(c)strcmp(c,'char'),type,'UniformOutput',false);
    charVars = vars([isChar{:}]); % These are the variables that are strings
    opts = setoptions(opts,charVars,'FillValue','null'); % New sql options
    data = fetch(conn,query,opts); % ,'DataReturnFormat','structure' - cant workout how to set this
    disp(data)
  catch exception
    close(conn);
    % setdbprefs(dbprefs);
  ...
  if isstruct(data)
    ...
    data = horzcat(data{:});
  elseif istable(data)
      % Import as table, default FE 19.01.2021
    if isempty(options.fields)
      fields = fieldnames(data);
    else
      fields = cellstr(options.fields(:));
