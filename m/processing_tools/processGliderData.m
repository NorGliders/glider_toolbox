...
%   NOTE:   %% Indentify samples where Cond=0 and Temp=0, on surface, may at begin of dive?  FE 2021.02.02
%  Syntax:
...
%  along with this program.  If not, see <http://www.gnu.org/licenses/>.
  ...
  %% Indentify samples where Cond=0 and Temp=0, on surface, may at begin of dive?  FE 2021.02.02
  if all(isfield(data_proc, {'conductivity' 'temperature'}))
    weird_CTD_values = (data_proc.temperature==0 & data_proc.conductivity==0);
    data_proc.conductivity(weird_CTD_values) = NaN;
    data_proc.temperature(weird_CTD_values) = NaN;
  end
